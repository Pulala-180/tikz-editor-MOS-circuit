...
describe("edit snapping core", () => {
  ...
  it("excludes open path bounds from reference snap targets when edit handles are not provided", () => {
    const scene = [
      makePath("open", [
        { kind: "M", to: wp(-20, 0) },
        { kind: "L", to: wp(20, 0) }
  ...
  it("adds path endpoint edit handles to reference snap targets", () => {
    ...
    const context = buildSnapContext({
      sceneElements: semantic.scene.elements,
      selectedSourceIds: [],
    ...
    const endpointKeys = context.referencePoints
      .filter((point) => point.role === "endpoint")
      .map((point) => `${point.sourceId}:${point.x.toFixed(6)}:${point.y.toFixed(6)}`);
    expect(endpointKeys).toContain("path:0:0.000000:0.000000");
    expect(endpointKeys).toContain("path:1:85.358268:0.000000");
  });

  it("adds selected path endpoints to movable selection snap points", () => {
    const source = String.raw`\begin{tikzpicture}
      \draw (0,0) -- (0.5,0.1) -- (1,0);
    \end{tikzpicture}`;
    const parsed = parseTikz(source, { recover: true });
    const semantic = evaluateTikzFigure(parsed.figure, source);

    const endpoints = collectPathEndpointSnapPoints(semantic.editHandles, new Set(["path:0"]));
    expect(endpoints.map((point) => point.role)).toEqual(["endpoint", "endpoint"]);
    expect(endpoints[0]).toMatchObject({ x: 0, y: 0 });
    expect(endpoints[1]?.x).toBeCloseTo(28.4527559, 6);
    expect(endpoints[1]?.y).toBeCloseTo(0, 6);

    const merged = mergeSnapPointLists([wp(0, 0), wp(5, 5)], endpoints);
    expect(merged).toHaveLength(3);
    expect(merged.some((point) => point.x === 0 && point.y === 0)).toBe(true);
    expect(merged.some((point) => Math.abs(point.x - 28.4527559) < 1e-6 && point.y === 0)).toBe(true);
  });

  it("locks horizontal resistor scopes to x movement but not plain lines", () => {
    const resistor = String.raw`\begin{tikzpicture}
      \begin{scope}[shift={(0,0)}]
        \coordinate (node_Rx.l) at (0,0);
        \draw[thick] (0,0) -- (0.155,0) -- (0.1875,0.15) -- (0.2525,-0.15) -- (0.3175,0.15) -- (0.3825,-0.15) -- (0.4475,0.15) -- (0.5125,-0.15) -- (0.545,0) -- (0.7,0);
        \coordinate (node_Rx.r) at (0.7,0);
      \end{scope}
    \end{tikzpicture}`;
    const parsed = parseTikz(resistor, { recover: true });
    const semantic = evaluateTikzFigure(parsed.figure, resistor);

    expect(resolveMoveAxisConstraintFromEditHandles(semantic.editHandles, ["path:1", "path:2", "path:3"])).toBe("x");
    expect(resolveMoveAxisConstraintFromEditHandles(semantic.editHandles, ["path:2"])).toBe("x");

    const plainLine = String.raw`\begin{tikzpicture}
      \draw (0,0) -- (2,0);
    \end{tikzpicture}`;
    const lineParsed = parseTikz(plainLine, { recover: true });
    const lineSemantic = evaluateTikzFigure(lineParsed.figure, plainLine);
    expect(resolveMoveAxisConstraintFromEditHandles(lineSemantic.editHandles, ["path:0"])).toBeNull();
  });


  it("treats invisible node ports as snappable endpoints", () => {
    const source = String.raw`\begin{tikzpicture}
      \begin{scope}[shift={(0.6,3.6)}]
        \node[draw=none, inner sep=0pt, outer sep=0pt] (node_Rx.l) at (0,0) {};
        \draw[thick] (0,0) -- (0.7,0);
        \node[draw=none, inner sep=0pt, outer sep=0pt] (node_Rx.r) at (0.7,0) {};
      \end{scope}
    \end{tikzpicture}`;
    const parsed = parseTikz(source, { recover: true });
    const semantic = evaluateTikzFigure(parsed.figure, source);
    ...
    const portKeys = context.referencePoints
      .filter((point) => point.role === "endpoint")
      .map((point) => `${point.x.toFixed(6)}:${point.y.toFixed(6)}`);
    expect(portKeys).toContain("17.071654:102.429921");
    expect(portKeys).toContain("36.988583:102.429921");
  });

  it("keeps unconnected resistors vertically movable until a wire is attached", () => {
    const resistor = String.raw`\begin{tikzpicture}
      \begin{scope}[shift={(0,0)}]
        \coordinate (node_Rx.l) at (0,0);
        \draw[thick] (0,0) -- (0.155,0) -- (0.1875,0.15) -- (0.2525,-0.15) -- (0.3175,0.15) -- (0.3825,-0.15) -- (0.4475,0.15) -- (0.5125,-0.15) -- (0.545,0) -- (0.7,0);
    ...
    expect(
      resolveMoveAxisConstraintFromEditHandles(
        ...
        ["scope:0", "path:1", "path:2", "path:3"],
        { requireAttachedWire: true, sceneElements: semantic.scene.elements }
      )
    ).toBeNull();

    const wired = String.raw`\begin{tikzpicture}
      \begin{scope}[shift={(0,0)}]
        \coordinate (node_Rx.l) at (0,0);
        \draw[thick] (0,0) -- (0.155,0) -- (0.1875,0.15) -- (0.2525,-0.15) -- (0.3175,0.15) -- (0.3825,-0.15) -- (0.4475,0.15) -- (0.5125,-0.15) -- (0.545,0) -- (0.7,0);
        \coordinate (node_Rx.r) at (0.7,0);
      ...
      \draw (0.7,0) -- (2,0);
    \end{tikzpicture}`;
    const wiredParsed = parseTikz(wired, { recover: true });
    const wiredSemantic = evaluateTikzFigure(wiredParsed.figure, wired);

    expect(
      resolveMoveAxisConstraintFromEditHandles(
        wiredSemantic.editHandles,
        ["scope:0", "path:1", "path:2", "path:3"],
        { requireAttachedWire: true, sceneElements: wiredSemantic.scene.elements }
      )
    ).toBe("x");
  });

  it("snaps a resistor port to another resistor port during selection drag", () => {
    const source = String.raw`\begin{tikzpicture}
\draw[thick] (0,0) -- (0.155,0) -- (0.1875,0.15) -- (0.2525,-0.15) -- (0.3175,0.15) -- (0.3825,-0.15) -- (0.4475,0.15) -- (0.5125,-0.15) -- (0.545,0) -- (0.7,0);
\draw[thick] (1,0) -- (1.155,0) -- (1.1875,0.15) -- (1.2525,-0.15) -- (1.3175,0.15) -- (1.3825,-0.15) -- (1.4475,0.15) -- (1.5125,-0.15) -- (1.545,0) -- (1.7,0);
\end{tikzpicture}`;
    const parsed = parseTikz(source, { recover: true });
    const semantic = evaluateTikzFigure(parsed.figure, source);

    const movingSourceIds = new Set(["path:1"]);
    const context = buildSnapContext({
      sceneElements: semantic.scene.elements,
      selectedSourceIds: [...movingSourceIds],
      editHandles: semantic.editHandles,
      zoom: 1,
      settings: { grid: { enabled: false }, gaps: { enabled: false } }
    });
    const bodyBounds = collectSourceWorldBounds(semantic.scene.elements).get("path:1");
    expect(bodyBounds).toBeTruthy();
    if (!bodyBounds) return;

    const selection = {
      bounds: bodyBounds,
      snapPoints: mergeSnapPointLists(
        selectionSnapPointsFromBounds(bodyBounds),
        collectPathEndpointSnapPoints(semantic.editHandles, movingSourceIds)
      )
    };

    // path:1 的左端点在 x=1cm，path:0 的右端点在 x=0.7cm。
    // 向左拖 0.4cm 时，左端点移动到 0.6cm；端点吸附应将其吸到
    // 0.7cm，即总位移为 -0.3cm。
    const snapped = snapSelectionTranslation({
      context,
      selection,
      rawDelta: wp(cm(-0.4), cm(0))
    });
    expect(snapped.snappedDelta?.x).toBeCloseTo(cm(-0.3), 6);
