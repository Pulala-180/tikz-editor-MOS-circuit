...
function getDraggedDomElements(
  draggedSourceIds: readonly string[],
  svgLayerHost: HTMLElement | null,
  interactionSvg: SVGSVGElement | null,
  scopeOverlay: ScopeOverlayIndex,
  sceneElements?: readonly SceneElement[]
): Element[] {
  const allSourceIds = collectSnapExcludedSourceIds(draggedSourceIds, scopeOverlay, sceneElements);
  const elements: Element[] = [];
  const visited = new Set<Element>();

  const addUnique = (el: Element | null) => {
  ...
  for (const sourceId of allSourceIds) {
    ...
    try {
      escaped = typeof CSS !== "undefined" && typeof CSS.escape === "function" ? CSS.escape(sourceId) : sourceId.replace(/"/g, '\\"');
    } catch {
      escaped = sourceId.replace(/"/g, '\\"');
    }

    if (svgLayerHost) {
      const svgNodes = svgLayerHost.querySelectorAll(`[data-source-id="${escaped}"]`);
      svgNodes.forEach(addUnique);
    }
    if (interactionSvg) {
      const overlayNodes = interactionSvg.querySelectorAll(
        `[data-source-id="${escaped}"],[data-selection-overlay-box-source-id="${escaped}"],[data-hit-region-target-id="${escaped}"]`
      );
      overlayNodes.forEach(addUnique);
    }
  }
...
export function useCanvasDragController(params: UseCanvasDragControllerParams) {
  ...
  useEffect(() => {
    ...
    function onWorldPointerMove(event: PointerEvent) {
      ...
      if (drag.kind === "element") {
        ...
        if (drag.movementAxis === "locked") {
          ...
          // 全局所有元件正交锁定：单次平移只能纯上下或者纯左右，严禁斜平移
          if (Math.abs(rawTotalDelta.x) >= Math.abs(rawTotalDelta.y)) {
            rawTotalDelta = makeWorldVector(rawTotalDelta.x, 0);
          } else {
            rawTotalDelta = makeWorldVector(0, rawTotalDelta.y);
          }
        ...
        const snapped = drag.snapContext && drag.initialSelection && drag.movementAxis !== "locked"
          ? snapSelectionTranslation({
              ...
              modifiers: { ctrlOrMeta },
              enabledAxis: drag.movementAxis === "orthogonal" ? undefined : drag.movementAxis ?? undefined
            })
          : {
        ...
        // Scope shifts are stored in pt.  When a resistor is axis-locked
        // (has attached wires) or a port point-snap fires, use fine (0.1pt)
        // formatting so the rewritten port stays within the wire-follow
        // matching tolerance of its target/attached wire.
        const moveFormatPrecision =
          (drag.movementAxis != null || snapped.lines.some((line) => line.type === "points")) &&
        ...
        drag.lastMoveFormatPrecision = moveFormatPrecision;

        // Apply transient visual transform directly to DOM elements (0ms AST / 0ms Re-render)
        if (!drag.transientDomElements) {
          drag.transientDomElements = getDraggedDomElements(
            drag.elementIds,
          ...
          drag.initialTransforms = new Map<Element, string | null>();
          for (const el of drag.transientDomElements) {
      ...
      if (!endpointAnchorOverlay?.snappedAnchor && drag.movementAxis && drag.lockedCoordinate != null) {
        if (drag.movementAxis === "x") {
          nextWorld = makeWorldPoint(nextWorld.x, drag.lockedCoordinate);
        } else if (drag.movementAxis === "y") {
          nextWorld = makeWorldPoint(drag.lockedCoordinate, nextWorld.y);
        }
      }
      ...
      if (drag.otherEndpointWorld) {
        ...
        const dx = nextWorld.x - drag.otherEndpointWorld.x;
        const dy = nextWorld.y - drag.otherEndpointWorld.y;
        const dist = Math.hypot(dx, dy);
        if (dist < MIN_WIRE_LENGTH_PT) {
          const origDx = drag.lastKnownWorld.x - drag.otherEndpointWorld.x;
          const origDy = drag.lastKnownWorld.y - drag.otherEndpointWorld.y;
          ...
          nextWorld = worldPoint(
            pt(drag.otherEndpointWorld.x + uX * MIN_WIRE_LENGTH_PT),
            pt(drag.otherEndpointWorld.y + uY * MIN_WIRE_LENGTH_PT)
          );
        }
      }
    ...
    function onWorldPointerUp(event: PointerEvent) {
      ...
      if (drag.kind === "element") {
        resetTransientDomTransforms(drag);
        if (Math.abs(drag.lastAppliedTotalDelta.x) > 1e-6 || Math.abs(drag.lastAppliedTotalDelta.y) > 1e-6) {
          applyActionWithFeedback(
            {
              kind: "moveElements",
              elementIds: drag.elementIds,
              delta: makeWorldPoint(drag.lastAppliedTotalDelta.x, drag.lastAppliedTotalDelta.y),
              formatPrecision: drag.lastMoveFormatPrecision
            },
            drag.historyMergeKey
          );
        }
    ...
    return () => {
      resetTransientDomTransforms(dragRef.current);
