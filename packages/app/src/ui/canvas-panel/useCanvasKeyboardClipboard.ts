...
export function useCanvasKeyboardClipboard(args: UseCanvasKeyboardClipboardArgs) {
  ...
  useEffect(() => {
    const handleWindowKeyDown = (event: KeyboardEvent) => {
      ...
      if (
        ...
        (target?.closest && target.closest("input, textarea, [contenteditable='true'], .monaco-editor, [data-modal='true']"))
      ) {
        return;
      }
      ...
      if (!event.ctrlKey && !event.metaKey && !event.altKey) {
        ...
        if (currentToolMode === "select") {
          ...
          // 未选中元件时按 V，记录 V 键按下状态以支持 VDD (按住V+D) 或召唤电压源
          if (key === "v") {
            vKeyDownRef.current = true;
            lastVKeyDownTimestampRef.current = Date.now();
          }
          ...
          if (key === "r") {
            const nextMode = rotateCircuitToolMode(currentToolMode);
            if (nextMode) {
            ...
            const nextMode = switchCircuitToolModeWithKey(currentToolMode, key);
            if (nextMode) {
              dispatch({ type: "SET_TOOL_MODE", mode: nextMode });
              event.preventDefault();
              return;
  ...
  }, [dispatch, setSnapLines, setToolCursorWorld, setToolDraft, setWarning]);
  ...
  const onViewportKeyDown = useCallback(
    (event: ReactKeyboardEvent<HTMLDivElement>) => {
      ...
      if (event.key === "Escape") {
        ...
        if (currentToolMode === "addFreehand") {
        ...
        if (currentToolMode === "addRoundedLine") {
        ...
        if (currentToolMode === "addOrthoWire") {
