...
export function createTemplateForToolDrag(
  ...
  if (mode === "addOrthoWire") {
    if (!hasDrag) {
      return { kind: "orthoWire", strokeColor };
    }
    ...
    if (dx >= dy) {
      ...
      p1 = worldPoint(pt(midX), pt(startWorld.y));
      p2 = worldPoint(pt(midX), pt(endWorld.y));
    } else {
      const midY = (startWorld.y + endWorld.y) / 2;
      p1 = worldPoint(pt(startWorld.x), pt(midY));
      p2 = worldPoint(pt(endWorld.x), pt(midY));
    }
    return {
      kind: "orthoWire",
      points: [p1, p2, endWorld],
      strokeColor
    };
  }
