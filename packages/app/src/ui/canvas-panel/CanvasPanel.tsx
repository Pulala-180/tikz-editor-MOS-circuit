...
export const CanvasPanel = memo(function CanvasPanel({
  ...
  const finalizePathDraft = useCallback(
    (closed: boolean) => {
      ...
      if (draft.appendTarget) {
        ...
        let snippet = generatePathToolSource(draft, { closed, strokeColor: creationStrokeColor });
        if (!snippet) {
          setPathDraft(null);
          setToolCursorWorld(null);
          dispatch({ type: "SET_TOOL_MODE", mode: "select" });
        ...
        if (toolMode === "addLine") {
          snippet = snippet.replace(/\\draw(\[.*?\])?/, (match, p1) => {
            if (p1) {
              return `\\draw[${p1.slice(1, -1)}, thick, line cap=round]`;
            }
            return `\\draw[thick, line cap=round]`;
          });
        }
    ...
    [applyActionWithFeedback, creationStrokeColor, dispatch, setDragState, toolMode]
  );
  ...
  const snapCrossSize = 6 / Math.max(canvasTransform.scale, 1e-3);
  const snapCrossStrokeWidth = 1.8 / Math.max(canvasTransform.scale, 1e-3);
  ...
  return (
    <>
      <CanvasPanelView
        ...
        snapCrossStrokeWidth={snapCrossStrokeWidth}
