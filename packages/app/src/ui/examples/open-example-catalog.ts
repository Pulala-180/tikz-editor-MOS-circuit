...
export const OPEN_EXAMPLE_CATALOG: readonly TikzOpenExample[] = [
  {
    id: "ai-live-neural-network",
    title: "AI Live Drawing: Deep Neural Network",
    description: "Multi-layer perceptron neural network with colored nodes and directed edges.",
    featureLabels: ["neural-network", "nodes", "loops", "layers"],
  ...
  % Title
  \node[font=\bfseries\Large, color=blue!70!black] at (4.5, 4.5) {Deep Neural Network Architecture};

  % Input Layer
  \foreach \i in {1,...,4} {
    \node[circle, draw=blue!80, fill=blue!15, minimum size=8mm, inner sep=0pt] (I-\i) at (0, 4.5 - \i*1.3) {$x_\i$};
  }

  % Hidden Layer 1
  \foreach \j in {1,...,5} {
    \node[circle, draw=orange!80, fill=orange!20, minimum size=8mm, inner sep=0pt] (H1-\j) at (3, 5.15 - \j*1.2) {$h^{(1)}_\j$};
  }

  % Hidden Layer 2
  \foreach \k in {1,...,5} {
    \node[circle, draw=purple!80, fill=purple!20, minimum size=8mm, inner sep=0pt] (H2-\k) at (6, 5.15 - \k*1.2) {$h^{(2)}_\k$};
  }

  % Output Layer
  \foreach \l in {1,...,2} {
    \node[circle, draw=green!60!black, fill=green!20, minimum size=8mm, inner sep=0pt] (O-\l) at (9, 3.8 - \l*1.4) {$y_\l$};
  }

  % Connections Input -> Hidden 1
  \foreach \i in {1,...,4} {
    \foreach \j in {1,...,5} {
      \draw[->, gray!50, line width=0.6pt] (I-\i) -- (H1-\j);
    }
  }
  ...
  \foreach \j in {1,...,5} {
    \foreach \k in {1,...,5} {
      \draw[->, gray!50, line width=0.6pt] (H1-\j) -- (H2-\k);
    }
  }
  ...
  \foreach \k in {1,...,5} {
    \foreach \l in {1,...,2} {
      \draw[->, gray!60, line width=0.8pt] (H2-\k) -- (O-\l);
    }
  }

  % Labels
  \node[above=0.3cm of I-1, font=\bfseries, color=blue!80] {Input Layer};
  \node[above=0.3cm of H1-1, font=\bfseries, color=orange!90!black] {Hidden Layer 1};
  \node[above=0.3cm of H2-1, font=\bfseries, color=purple!80] {Hidden Layer 2};
  \node[above=0.3cm of O-1, font=\bfseries, color=green!60!black] {Output Layer};
