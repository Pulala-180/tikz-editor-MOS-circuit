...
const PostSaveDraftModal = lazy(async () => {
  const mod = await import("./PostSaveDraftModal");
  return { default: mod.PostSaveDraftModal };
});
...
export function App() {
  ...
  const [pendingPostSaveDraft, setPendingPostSaveDraft] = useState<{
    documentId: string;
  ...
  useEffect(() => {
    ...
    globalLike.__TIKZ_EDITOR_APP_TEST_API__ = {
      ...
      getToolMode: () => {
        return useEditorStore.getState().toolMode;
      },
  ...
  async function handleUnsavedDecision(
    ...
    if (decision === "save-to-sketch") {
      setPendingClose(null);
      for (const documentId of closeCtx.dirtyDocumentIds) {
        const doc = documents[documentId];
        if (doc && typeof import.meta !== "undefined" && import.meta.hot) {
  ...
  function handlePostSaveDraftDecision(deleteDraft: boolean): void {
    ...
    setPendingPostSaveDraft(null);
    executeCloseIntent(closeIntent);
  }
  ...
  return (
    ...
    >
      ...
      {pendingClose ? (
        <Suspense fallback={null}>
          <UnsavedChangesModal
            ...
            isExternal={pendingClose.dirtyDocumentIds.some((id) => Boolean(documents[id]?.fileRef != null))}
            onChoose={(decision) => {
      ...
      {pendingPostSaveDraft ? (
        <Suspense fallback={null}>
          <PostSaveDraftModal
            documentTitle={pendingPostSaveDraft.documentTitle}
            onChoose={(deleteDraft) => {
              handlePostSaveDraftDecision(deleteDraft);
            }}
          />
        </Suspense>
      ) : null}
