...
export function createEditorCommandRuntime(input: RuntimeInput): EditorCommandRuntime {
  ...
  const runOpenDocument = (options: { requireSvg?: boolean; requireIpe?: boolean; addToRecent?: boolean } = {}) => {
    ...
    void openText({ addToRecent: options.addToRecent ?? true }).then(async (opened) => {
      ...
      dispatch({ type: "NEW_DOCUMENT", source: resolved.source, title: resolved.title, isExternal: true });
  ...
  const bindings: CommandBindings = {
    ...
    [APP_MENU_COMMAND_IDS.CLEAR_CANVAS]: {
      enabled: true,
      run: () => {
        dispatch({
          type: "CODE_EDITED",
        ...
        if (typeof window !== "undefined") {
          try {
            localStorage.removeItem("tikz-editor:workspace");
            localStorage.removeItem("tikz-editor:workspace-v4");
          } catch {}
        }
        if (typeof import.meta !== "undefined" && import.meta.hot) {
          import.meta.hot.send?.("agent:clear-code");
        }
      }
    },
