...
function nextAvailableName(baseName: string, usedNames: ReadonlySet<string>): string {
  ...
  if (normalized.includes(".")) {
    ...
    const stemPart = normalized.slice(0, dotIndex);
    const suffixPart = normalized.slice(dotIndex);
    const digitSuffix = /^(.*?)(\d+)$/u.exec(stemPart);
    const stem = digitSuffix && digitSuffix[1].length > 0 ? digitSuffix[1] : stemPart;
    let counter = digitSuffix ? Number.parseInt(digitSuffix[2], 10) + 1 : 2;
    if (!Number.isFinite(counter) || counter < 2) {
      counter = 2;
    }

    let candidate = `${stem}${counter}${suffixPart}`;
    while (usedNames.has(candidate)) {
      counter += 1;
      candidate = `${stem}${counter}${suffixPart}`;
    }
    return candidate;
  }
