...
export { resolveMoveAxisConstraintFromEditHandles };
...
function collectPointGridAndGapSnaps({
  ...
  if (settings.grid.enabled) {
    const hasPointX = nearest.x.some((snap) => snap.kind === "point");
    const hasPointY = nearest.y.some((snap) => snap.kind === "point");
    let gridAxis: Axis | null = enabledAxis ?? null;
    if (hasPointX && hasPointY) {
      gridAxis = null;
    } else if (hasPointX) {
      gridAxis = enabledAxis === "x" ? null : "y";
    } else if (hasPointY) {
      gridAxis = enabledAxis === "y" ? null : "x";
    }

    if (gridAxis !== null || (!hasPointX && !hasPointY && enabledAxis === null)) {
      collectGridSnaps({
        selectionPoints: selection.snapPoints,
        minOffset,
        nearest,
        gridStep: pickGridStepPt(context.zoom, settings.grid.minorTargetPx),
        enabledAxis: gridAxis
      });
    }
  }
