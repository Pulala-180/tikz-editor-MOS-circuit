...
export type ElementTemplate =
  ...
  | { kind: "orthoWire"; points?: WorldPoint[]; fromAnchor?: AnchorReference; toAnchor?: AnchorReference; strokeColor?: string }
  ...
  | { kind: "resistor"; to?: WorldPoint; strokeColor?: string };
...
export function generateElementSource(template: ElementTemplate, at: WorldPoint): string {
  ...
  switch (template.kind) {
    ...
    case "orthoWire": {
      const fromCoord = formatLineEndpoint(template.fromAnchor, atCoord);
      const points = template.points && template.points.length >= 3
        ? template.points
        : [
            wp(at.x + DEFAULT_LINE_LENGTH_PT / 3, at.y),
            wp(at.x + DEFAULT_LINE_LENGTH_PT / 3, at.y + DEFAULT_LINE_LENGTH_PT / 3),
            wp(at.x + DEFAULT_LINE_LENGTH_PT, at.y + DEFAULT_LINE_LENGTH_PT / 3)
          ];
      const p1 = formatPointCm(points[0]);
      const p2 = formatPointCm(points[1]);
      const p3 = formatLineEndpoint(template.toAnchor, formatPointCm(points[points.length - 1]));
      const lineOptions = buildDrawOptions(template.strokeColor, undefined, false);
      return `\\draw${lineOptions} ${fromCoord} -- ${p1} -- ${p2} -- ${p3};`;
    }
    ...
    case "resistor": {
      const to = template.to ?? wp(at.x + 4 * PT_PER_CM, at.y);
      const drawOptions = buildDrawOptions(template.strokeColor, undefined, false);
      const dx = to.x - at.x;
      const dy = to.y - at.y;
      const len = Math.hypot(dx, dy);
      if (len < 1e-4) {
        return `\\draw${drawOptions} ${atCoord} -- ${formatPointCm(to)};`;
      }
      const ux = dx / len;
      const uy = dy / len;
      const px = -uy;
      ...
      const segLen = len / 8;
      const amp = len * 0.075;
      const points: string[] = [atCoord];
      for (let i = 0; i < 8; i++) {
        const frac = (i + 1) / 8;
        const cx = at.x + dx * frac;
        const cy = at.y + dy * frac;
        if (i >= 1 && i <= 6) {
          const side = i % 2 === 1 ? 1 : -1;
          const mx = at.x + dx * ((i + 0.5) / 8) + px * amp * side;
          const my = at.y + dy * ((i + 0.5) / 8) + py * amp * side;
          points.push(formatPointCm(wp(mx, my)));
        }
        points.push(formatPointCm(wp(cx, cy)));
      }
      return `\\draw${drawOptions} ${points.join(" -- ")};`;
    }
  }
}
