...
export function applyMoveElementsAction(
  ...
  for (const scopeId of scopeElementIdSet) {
    ...
    if (scope) {
      collectAllDescendantStatementIds(scope.body, scopeDescendantIdSet);
    }
  }

  const topLevelScopeElementIds = [...scopeElementIdSet].filter(
    (scopeId) => !scopeDescendantIdSet.has(scopeId)
  );

  delta = clampDeltaForAttachedWires(
    source,
    editHandles,
  ...
  const nonMatrixElementIds = normalizedIds.filter(
    (elementId) =>
      ...
      !scopeDescendantIdSet.has(elementId) &&
      !treeRootElementIdSet.has(elementId)
  );
  ...
  if (topLevelScopeElementIds.length > 0) {
  ...
  // 橡皮筋导线：scope 的 \coordinate 端口、或直接移动的路径端点移动后，
  // 与其 world 坐标重合的顶层导线端点跟随移动（导线自动拉长/变短）。
  // wire-follow 基于原始 source 生成原始空间的改写，这里链式应用到
  // currentSource（偏移 = 位于其之前的 patch 长度差之和），patch 的 oldSpan
  // 保持原始空间——与其余 patch 同基准，normalizeResultPatches 不会退化为整块
  // 替换。导线 id 必须进 changedSourceIds，否则增量重算范围缺它、逐帧 handle
  // 陈旧漂移。
  const pathPointSourceIds = new Set<string>();
  for (const handle of editHandles) {
    if (handle.kind === "path-point") {
      pathPointSourceIds.add(handle.sourceRef.sourceId);
    }
  }
  ...
  if (shouldApplyWireFollow) {
    ...
    for (const pending of sortedPending) {
      ...
      for (const patch of patches) {
        if (patch.oldSpan.to <= pending.span.from) {
          shift += patch.replacement.length - (patch.oldSpan.to - patch.oldSpan.from);
        }
      }
      const applied = replaceSpan(
        currentSource,
        { from: pending.span.from + shift, to: pending.span.to + shift },
        pending.text
      );
      patches.push({
        oldSpan: pending.span,
...
function applyMoveElementsUsingHandleRewrites(
  ...
  // When moving a path-point handle (such as IO node hollow circle), also shift any companion text node in close proximity
  if (rewritable.length === 1 && rewritable[0].kind === "path-point") {
    const target = rewritable[0];
    for (const h of editHandles) {
      if (h.kind === "node-position" && h.id !== target.id) {
        const dx = h.world.x - target.world.x;
        const dy = h.world.y - target.world.y;
        if (Math.hypot(dx, dy) <= 30) {
          const actualText = source.slice(h.sourceRef.sourceSpan.from, h.sourceRef.sourceSpan.to);
          if (actualText === h.sourceText) {
            const newWorld = worldPoint(pt(h.world.x + delta.x), pt(h.world.y + delta.y));
            const text = rewriteCoordinate(newWorld, h, source);
            if (text != null) {
              pending.push({ span: h.sourceRef.sourceSpan, text });
            }
          }
        }
      }
...
function rewriteSingleScopeTransform(
  ...
  const entries = targetOptionsEntries(resolved.target);
  const shiftEntries = entries.filter((entry): entry is KeyValueOptionCandidate["entry"] => {
  ...
  const xyTranslationEntries = entries.filter((entry) => {
    if (entry.kind !== "kv") return false;
    const key = normalizeOptionKey(entry.key);
    return key === "xshift" || key === "/tikz/xshift" || key === "yshift" || key === "/tikz/yshift";
  });

  if (lastShift && parseShiftTransformValue(lastShift.valueRaw) == null && xyTranslationEntries.length === 0) {
    return {
      kind: "unsupported",
      reason: `Scope ${elementId} is anchored by a coordinate reference and cannot be moved directly`
    };
  }
...
function rewriteSingleScopeShiftInPlace(
  ...
  const lastShift = shiftEntries[shiftEntries.length - 1] ?? null;
  const isNumericShift = lastShift && parseShiftTransformValue(lastShift.entry.valueRaw) != null;
  if (!isNumericShift) {
    ...
    const isAnchoredScope = lastShift != null && parseShiftTransformValue(lastShift.entry.valueRaw) == null;
    ...
    if (!isAnchoredScope || hasYShiftEntry) {
...
function collectAllDescendantStatementIds(statements: readonly Statement[], out: Set<string>): void {
  for (const statement of statements) {
    out.add(statement.id);
    if (statement.kind === "Scope") {
      collectAllDescendantStatementIds(statement.body, out);
    }
  }
}
...
function expandDirectlyConnectedScopes(
  statements: readonly Statement[],
  editHandles: readonly EditHandle[],
  scopeElementIdSet: Set<string>
): void {
  if (scopeElementIdSet.size === 0) return;
  const MATCH_EPSILON_PT = 0.5;

  const allScopes = new Map<string, Extract<Statement, { kind: "Scope" }>>();
  collectAllScopes(statements, allScopes);

  const portStatementIdsByScope = new Map<string, Set<string>>();
  for (const [scopeId, scope] of allScopes.entries()) {
    const portIds = new Set<string>();
    collectScopeCoordinateStatementIds(scope.body, portIds);
    portStatementIdsByScope.set(scopeId, portIds);
  }

  let expanded = true;
  while (expanded) {
    expanded = false;
    const movedPortWorlds: WorldPoint[] = [];
    for (const scopeId of scopeElementIdSet) {
      const portIds = portStatementIdsByScope.get(scopeId);
      if (!portIds) continue;
      for (const handle of editHandles) {
        if (handle.kind === "path-point" && portIds.has(handle.sourceRef.sourceId)) {
          movedPortWorlds.push(handle.world);
        }
      }
    }

    if (movedPortWorlds.length === 0) break;

    for (const [scopeId] of allScopes.entries()) {
      if (scopeElementIdSet.has(scopeId)) continue;
      const portIds = portStatementIdsByScope.get(scopeId);
      if (!portIds) continue;

      let touchesMovedScope = false;
      for (const handle of editHandles) {
        if (handle.kind === "path-point" && portIds.has(handle.sourceRef.sourceId)) {
          if (
            movedPortWorlds.some(
              (p) =>
                Math.abs(p.x - handle.world.x) <= MATCH_EPSILON_PT &&
                Math.abs(p.y - handle.world.y) <= MATCH_EPSILON_PT
            )
          ) {
            touchesMovedScope = true;
            break;
          }
        }
      }

      if (touchesMovedScope) {
        scopeElementIdSet.add(scopeId);
        expanded = true;
      }
    }
  }
...
function collectAllScopes(
  statements: readonly Statement[],
  out: Map<string, Extract<Statement, { kind: "Scope" }>>
): void {
  for (const statement of statements) {
    if (statement.kind === "Scope") {
      out.set(statement.id, statement);
      collectAllScopes(statement.body, out);
    }
  }
}

function collectScopeCoordinateStatementIds(
  statements: readonly Statement[],
  out: Set<string>
): void {
  for (const statement of statements) {
    if (statement.kind === "Path") {
      ...
      collectScopeCoordinateStatementIds(statement.body, out);
    }
  }
}
