...
export function collectNodeAnchorTargets(context: SemanticContext): NodeAnchorTarget[] {
  ...
  for (const [coordinateName, world] of context.namedCoordinates) {
    let nodeName = coordinateName;
    let anchor = "center";
    const dot = coordinateName.indexOf(".");
    if (dot > 0 && dot < coordinateName.length - 1) {
      nodeName = coordinateName.slice(0, dot);
      anchor = coordinateName.slice(dot + 1).trim();
    }
    if (anchor.length === 0) {
      anchor = "center";
    }
  ...
  for (const [nodeName, anchors] of anchorsByNode) {
    for (const entry of anchors) {
      const normalizedAnchor = entry.anchor.trim().toLowerCase();
      const key = `${nodeName}\u0000${normalizedAnchor}`;
      if (seen.has(key)) {
        continue;
      }
